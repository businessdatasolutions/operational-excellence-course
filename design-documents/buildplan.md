...
- [x] **3.7** Koppel `DatabaseSessionService` (uit `shared/session.py`) met één sessie per team-per-checkpoint. ✅ `agent.py`'s `session_id_for(team_id, stage)` (`f"{team_id}--{stage}"`) + `build_session_service()` roepen `shared/session.py`'s bestaande env-flag aan; deze devomgeving heeft geen `DATABASE_URL` (nog geen managed Postgres — Task 7), dus tests/demo draaien tegen de `InMemorySessionService`-fallback — de omschakeling naar `DatabaseSessionService` is enkel een env-var-flip, geen codewijziging.
...
- [x] **4.3** Implementeer fallback-herconfiguratie (FR-08): een functie `reconfigure_ring(ring_id, departed_team_id)` die alleen de betrokken posities aanpast. ✅ Werkelijke signatuur `reconfigure_ring(root, cohort, klas_id, ring_id, departed_team_id)` — `ring_id`/`departed_team_id` letterlijk zoals hierboven gespecificeerd, plus de drie parameters die nodig zijn om het bestand daadwerkelijk te lokaliseren (dat is prose-kortschrift in de subtaaktekst, geen volledige signatuur). Listdeletie herverbindt voorganger→opvolger vanzelf; geen andere ring in de klas wordt gelezen of geschreven (apart getest). **Live afgestemd met de Task 9-agent** (team-roster↔ring-koppeling, 9.4): zij lazen deze module rechtstreeks en wiring inmiddels op de echte signatuur, bevestigd over-en-weer.
...
- [x] **10.4** Bevestig instructor-only-toegang: dezelfde toegangsscheiding als `rubric_scores` — geen teamgericht kanaal geeft ooit toegang tot dit dossier. ✅ Dezelfde code-pad-scheiding als Task 3's `rubric_scores`: de drie nieuwe tools zitten uitsluitend in `dashboard_query.prep_agent`/`agent.py`; `socratic_tutor` (het enige teamgerichte kanaal in dit codebase) heeft geen enkele import ervan — geverifieerd via echte `ast`-parsing van importstatements (niet een kwetsbare substring-scan op docstrings), in beide richtingen getest.
